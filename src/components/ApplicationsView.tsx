...
import { Layers, Plus, Copy, Check, ExternalLink, ShieldCheck, Trash2 } from 'lucide-react';

interface ApplicationsViewProps {
  applications: Application[];
  onAddApp: (name: string, version: string, downloadLink: string) => void;
}
...
export const ApplicationsView: React.FC<ApplicationsViewProps> = ({ applications, onAddApp }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name) return;
    onAddApp(name, version, downloadLink);
    setName('');
    setVersion('1.0.0');
    setDownloadLink('');
    setShowModal(false);
